/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   exec_func_tools.c                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ubuntu <ubuntu@student.42.fr>              +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/22 18:05:19 by subpark           #+#    #+#             */
/*   Updated: 2024/01/21 15:01:23 by ubuntu           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../include/minishell.h"

char	*paths_line(char **envp)
{
	int		i;
	char	*paths;

	i = 0;
	while (envp[i] != NULL)
	{
		if (f_strcmp("PATH=", envp[i]) == 0)
		{
			paths = envp[i];
			return (paths);
		}
		i++;
	}
	return (NULL);
}

int	path_len(char *s)
{
	int	i;

	i = 0;
	while (s[i] != '\0' && s[i] != ':' && s[i] != '\n')
		i++;
	return (i);
}

char	*command_path(char **path_array, int i, char *command)
{
	char	*part_path;
	char	*path;

	part_path = ft_strjoin(path_array[i], "/");
	path = ft_strjoin(part_path, command);
	free(part_path);
	return (path);
}

int	f_extract_command(char *str)
{
	int	i;

	i = 0;
	while (str[i] != '\0' && str[i] != ' ')
		i++;
	return (i);
}

char	*command_to_path(char *argv, int len, char *string, int i)
{
	int		k;
	int		comm;
	char	*s;

	k = 0;
	comm = f_extract_command(argv) + 1;
	s = malloc(len + comm + 2);
	while (len > 0)
	{
		s[k] = string[i];
		k++;
		i++;
		len--;
	}
	s[k] = '/';
	k++;
	i = 0;
	while (--comm > 0)
	{
		s[k] = argv[i];
		k++;
		i++;
	}
	s[k] = '\0';
	return (s);
}

char	*paths_to_array(char *string, int path_count, char *argv)
{
	int		i;
	int		m;
	int		len;
	char	*command;

	i = 0;
	m = 0;
	len = 1;
	while (path_count > 0)
	{
		len = path_len(string + i);
		command = command_to_path(argv, len, string, i);
		if (access(command, F_OK) == 0 && access(command, X_OK) == 0)
			return (command);
		i += len + 1;
		path_count--;
		m++; 
	}
	return (NULL);
}

int	path_counter(char *line)
{
	int	i;
	int	k;

	k = 0;
	i = 0;
	while (line[i] != '\0')
	{
		if (line[i] == ':')
			k++;
		i++;
	}
	return (k + 1);
}

char	*commandret(char *argv, int path_count, char *path)
{
	int		i;
	char	*command;

	i = 0;
	if (access(argv, F_OK) == 0 && access(argv, X_OK) == 0)
		return (argv);
	while (path[i] != '/' && path[i] != '\0')
		i++;
	if (path[i] == '\0')
		return (NULL);
	command = paths_to_array(path + i, path_count, argv);
	return (command);
}

char	*path_pointer(char **envp, t_envp *env)
{
	char	*path;
	char	*command;
	int		i;

	path = paths_line(env->envp);
	i = path_counter(path);
	command = commandret(envp[0], i, path);
	return (command);
}

/* char	*path_pointer(char **envp, char *command)
{
	char	*path;
	int		i;

	i = 0;
	while (envp[i] != NULL)
	{
		if (command[0] != '/')
			path = command_path(envp, i, command);
		else
			path = ft_strdup(command);
		if (access(path, F_OK) == 0 && access(path, X_OK) == 0)
			return (path);
		free(path);
		i ++;
	}
	return (NULL);
} */

void	exec(char **cmd, t_envp *env)
{
	char	*path;
	int		g_exit_status;

	if (!cmd || !cmd[0])
		exit(0);
	path = path_pointer(cmd, env);
	if (!path)
		exit(2);
	g_exit_status = execve(path, cmd, env->envp);
	if (path)
		free(path);
	g_exit_status = errno;
	if (g_exit_status)
		exit (errno);
}

int	redirect_type(t_cmd *node)
{
	if (ft_strcmp(node->cmdstr[0], "<") == 0)
		return (REL_TYPE_L);
	else if (ft_strcmp(node->cmdstr[0], "<<") == 0)
		return (REL_TYPE_LL);
	else if (ft_strcmp(node->cmdstr[0], ">") == 0)
		return (REL_TYPE_R);
	else if (ft_strcmp(node->cmdstr[0], ">>") == 0)
		return (REL_TYPE_RR);
	return (0);
}

/*
command_path:
for the path_pointer check whether that command is exist or not(as a file),
make command as a type of path.
it can be existing/non-existing path.

path_pointer:
function that returns path of command exist.
from the path generated by command_path,
*/